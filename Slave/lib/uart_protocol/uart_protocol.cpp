...
{
    // Packet: [J][algo][dataHi][dataLo][keyHi][keyLo][data...][key...][CRC-8]
    ...
    for (int attempt = 0; attempt < UART_MAX_RETRIES; ++attempt) {
        ...
        // ── 2. Fixed header ─────────────────────────────────────────────
        uint8_t hdr[5]; // [algo][dataHi][dataLo][keyHi][keyLo]
        if (!readExact(hdr, sizeof(hdr))) continue;

        uint16_t dataSize = ((uint16_t)hdr[1] << 8) | hdr[2];
        uint16_t keySize  = ((uint16_t)hdr[3] << 8) | hdr[4];

        if (dataSize > UART_MAX_DATA_SIZE || keySize > UART_MAX_KEY_SIZE) {
            while (_serial.available()) _serial.read();
            sendAck(false);
