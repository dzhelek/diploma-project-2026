...
#define UART_RX_BUFFER_SIZE   (17408)   // >= largest frame (~16.4 KB) so a WiFi/MQTT stall can't drop RX bytes
// NOTE: non-ESP32 slaves (ESP-12 / Pico / Nano) must also use UART_BAUD on their link UART,
//       and their monitor_speed updated to match, before they can talk to this master.
...
struct ResponsePacket {
    uint32_t    timeUs;                         // Processing time in microseconds
    uint16_t    dataSize;                       // Number of valid bytes in data[]
    uint8_t*    data;
};
